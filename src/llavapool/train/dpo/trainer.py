...
class CustomDPOTrainer(DPOTrainer):
    ...
    def get_batch_loss_metrics(
        ...
        metrics[f"{prefix}logps/margins"] = (policy_chosen_logps - policy_rejected_logps).mean().item()
        metrics[f"{prefix}logits/chosen"] = policy_chosen_logits.mean().item()
        metrics[f"{prefix}logits/rejected"] = policy_rejected_logits.mean().item()
        metrics[f"{prefix}logits/margins"] = (policy_chosen_logits - policy_rejected_logits).mean().item()

        if reference_chosen_logps is not None and reference_rejected_logps is not None:
            metrics[f"{prefix}ref_logps/chosen"] = reference_chosen_logps.mean().item()
            metrics[f"{prefix}ref_logps/rejected"] = reference_rejected_logps.mean().item()
            metrics[f"{prefix}ref_logps/margins"] = (reference_chosen_logps - reference_rejected_logps).mean().item()
